...
long int getMatrix(){
	if ( m == 0 ) {
		Matrix	= ReadZZ( ReadZZ( BaseMatrix2 ) + 0x50 );
		Matrixa = Matrix + 0xC0;
		printf( "扫描到矩阵0x%lX\n", Matrixa );
		m = 1;
		return(Matrixa);
...
int main( int argc, char **argv ){
	...
	BaseMatrix2	= getModuleBase( (char*)"libunity.so", 1 ) + 0x140B7A0;
	float py = ReadDword( BaseMatrix + 0x14560A0 ) / 2;      /* 1080 */
	float px = ReadDword( BaseMatrix + 0x145609C ) / 2;      /* 2340 在cb */
	long lib = getModuleBase( (char*)"libGameCore.so", 1 ) + 0x426E038;
	long libbuff = getModuleBase( (char*)"libGameCore.so", 1 ) + 0x4042190;
	long 开始 = getModuleBase( (char*)"libGameCore.so", 1 ) + 0x4496538;
	FILE *fp;
	while (1) {
		if ( ReadDword( 开始 ) == 1 ) {
			printf("对局进行中\n");
			...
			for ( int i = 0; i < 数量; i++ ) {
				long Array_structure = ReadZZ( ReadZZ( ReadZZ( lib ) + 0x228 ) + 0x18 * i );
				rd = ReadDword( Array_structure + 0x2c );
				if ( rd == my_rd ){
					continue;
				}
				int id = ReadDword( Array_structure + 0x20 );
				Coordinate_addr = ReadZZ( ReadZZ( ReadZZ( Array_structure + 0x1D8 ) + 0x10 ) + 0x0 ) + 0x10;
				for ( int a = 0; a < 2; a++ ){
				...
				int 回城 = ReadDword( (ReadZZ( ReadZZ( Array_structure + 0xF8 ) + 0x150 ) + 0xA0) - 0x160 );
				...
				for ( long i = 0; i < 野怪数量; i++ ) {
					long buffarryaddr = ReadZZ( ReadZZ( Buff_addr ) + (i * 0x18) );
					buff_id = ReadDword( buffarryaddr + 0xC0 );
					buff_team = ReadDword( buffarryaddr + 0x28C );
					if ( buff_id == 6010 && buff_team > 0 )
						蓝bufftime1 = ReadDword( buffarryaddr + 0x220 ) / 1000;
					else if ( buff_id == 6010 && buff_team < 0 )
						蓝bufftime2 = ReadDword( buffarryaddr + 0x220 ) / 1000;
					else if ( buff_id == 6011 && buff_team > 0 )
						红bufftime1 = ReadDword( buffarryaddr + 0x220 ) / 1000;
					else if ( buff_id == 6011 && buff_team < 0 )
						红bufftime2 = ReadDword( buffarryaddr + 0x220 ) / 1000;
					else
						continue;
				}
			}
